...
from model.deeplab3 import Deeplabv3p
...
def CreateModel(args):
    ...
    if args.model == 'DeepLabv3p':
        model = Deeplabv3p(num_classes=args.num_classes, init_weights=args.init_weights, restore_from=args.restore_from, phase=args.set, output_stride=args.output_stride)
        if args.set == 'train':
            optimizer = optim.SGD(model.optim_parameters(args),
                                  lr=args.learning_rate, momentum=args.momentum, weight_decay=args.weight_decay)
            optimizer.zero_grad()
